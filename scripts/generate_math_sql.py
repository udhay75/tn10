#!/usr/bin/env python3
"""
Generate supabase/migrations/004_seed_math_2025.sql from src/data/class_10_math_2025.json
"""
...
with open('src/data/class_10_math_2025.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

sql_lines = []
sql_lines.append("-- ==========================================================================")
sql_lines.append("-- 004_seed_math_2025.sql: Tamil Nadu State Board Class 10 Mathematics Seed")
sql_lines.append("-- Idempotent seed script: safe to run multiple times without duplicates.")
sql_lines.append("-- ==========================================================================")
sql_lines.append("")

# Constraint update
sql_lines.append("-- 1. Expand lesson_type check constraint for multi-subject support")
sql_lines.append("ALTER TABLE lessons DROP CONSTRAINT IF EXISTS lessons_lesson_type_check;")
sql_lines.append("ALTER TABLE lessons ADD CONSTRAINT lessons_lesson_type_check ")
sql_lines.append("  CHECK (lesson_type IN ('prose', 'poem', 'supplementary', 'theory', 'exercise', 'practical', 'review'));")
sql_lines.append("")

# Subject
sql_lines.append("-- 2. Mathematics Subject")
...
VALUES ('tn10_math_2025_edition', 'class_10_math', 2025, {escape_sql(data['subject']['textbook']['title'] + ' (' + data['subject']['textbook']['edition'] + ')')}, 'Class_10_Mathematics_English_2025_Edition.pdf', true)
ON CONFLICT (code) DO UPDATE SET title = EXCLUDED.title, is_published = EXCLUDED.is_published;""")
sql_lines.append("")

# Units, Lessons, and Items
...
VALUES ({escape_sql(unit['id'])}, 'tn10_math_2025_edition', {unit['unit_number']}, {escape_sql(unit['title'])}, {escape_sql(unit['theme'])}, {u_idx})
...
output_sql = 'supabase/migrations/004_seed_math_2025.sql'
