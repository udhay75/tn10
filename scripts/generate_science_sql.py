#!/usr/bin/env python3
"""
Generate supabase/migrations/005_seed_science_2024.sql from src/data/class_10_science_2024.json
"""
...
with open('src/data/class_10_science_2024.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

sql_lines = []
sql_lines.append("-- ==========================================================================")
sql_lines.append("-- 005_seed_science_2024.sql: Tamil Nadu State Board Class 10 Science Seed")
sql_lines.append("-- Idempotent seed script: safe to run multiple times without duplicates.")
sql_lines.append("-- ==========================================================================")
sql_lines.append("")

# 1. Science Subject
sql_lines.append("-- 1. Science Subject")
...
VALUES ('tn10_sci_2024_edition', 'class_10_science', 2024, {escape_sql(data['subject']['textbook']['title'] + ' (' + data['subject']['textbook']['edition'] + ')')}, 'Class_10_Science_English_2024_Edition.pdf', true)
...
VALUES ({escape_sql(unit['id'])}, 'tn10_sci_2024_edition', {unit['unit_number']}, {escape_sql(unit['title'])}, {escape_sql(unit['theme'])}, {u_idx})
...
output_sql = 'supabase/migrations/005_seed_science_2024.sql'
